...
async def start_cmd(message: types.Message, command: CommandObject):
    ...
    if is_new:
        db["users"][user_id] = {"balance": 0, "invites": 0, "username": message.from_user.username or "بدون", "first_name": message.from_user.first_name or ""}
        ...
        save_db(db)
        try:
            new_user_text = f"👤 مستخدم جديد\n🆔: `{message.from_user.id}`\n👤: {message.from_user.first_name}\n@{message.from_user.username or 'بدون'}\nدعوة: {args or 'مباشر'}\nالاجمالي: {len(db['users'])}"
            kb_new = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🚫 حظره", callback_data=f"ban_quick_{message.from_user.id}")]])
    ...
    if not await check_join(message.from_user.id):
        ...
        return
    await message.answer(f"مرحبا {message.from_user.first_name} 👋\nرصيدك: {db['users'][user_id]['balance']} ⭐", reply_markup=main_keyboard())
...
async def cat_handler(call: types.CallbackQuery):
    if call.data == "cat_clean":
        ...
        await call.message.edit_text("⚠️ أرقام السبام\nسعر رخيص بس محظورة مؤقت:", reply_markup=countries_keyboard("spam"))
...
async def buy_country(call: types.CallbackQuery):
    ...
    await bot.send_invoice(chat_id=call.from_user.id, title=f"شراء رقم {name}", description=f"الدولة: {name}\nالسعر: {price} ⭐", payload=f"order_{cat}_{name}_{price}", provider_token="", currency="XTR", prices=[LabeledPrice(label=f"{name}", amount=price)])
    await call.answer()
...
async def deposit_choose_currency(call: types.CallbackQuery, state: FSMContext):
    ...
    if "GRAM" in currency:
        text += "🏷️ الميمو تاج:\n`10051143`\n\n🚨 تحذير: لازم تكتب الميمو تاج والا فلوسك هتضيع ❌\n\n"
    text += "💰 الاسعار:\n15 ⭐ = 0.12$\n25 ⭐ = 0.22$\n50 ⭐ = 0.42$\n100 ⭐ = 0.90$\n\n✍️ اكتب كمية النجوم:"
...
async def deposit_accept(call: types.CallbackQuery):
    ...
    db["users"][uid]["balance"] += dep["amount"]
    dep["status"] = "accepted"
    save_db(db)
